...
################################################################################
sudo mkfifo $TMP/mcs_a1 $TMP/mcs_v1
...
do
    echo cqd i = $i
	sudo mkfifo $TMP/mcs_a$i $TMP/mcs_v$i
	sudo chmod 777 $TMP/mcs_a$i
	sudo chmod 777 $TMP/mcs_v$i

	sudo ffmpeg -y -i $f -vn -f u16le -acodec pcm_s16le -ac 2 -ar 44100 $TMP/mcs_a$i 2>/dev/null </dev/null &
...
################################################################################
sudo mkfifo $TMP/mcs_a_all
sudo mkfifo $TMP/mcs_v_all

sudo chmod 777 $TMP/mcs_a_all
sudo chmod 777 $TMP/mcs_v_all

echo cqd $all_a
echo cqd $all_v

sudo cat $all_a > $TMP/mcs_a_all &
sudo cat $all_v > $TMP/mcs_v_all &
